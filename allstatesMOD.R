...
##The following lines use rvest to extract the nodes I want, convert to text, make into data frame, and bind to the results of the previous loops. Following Hadley Wickam's suggestion, I started by using Chrome's "SelectorGadget" to get CSS elements, but ran into some major limitations of that tool. For example, metadata that is not visible on the page can not be selected using that tool (as in the case of the 'startdate' variable below, which I had to manually write the selector for). It also has problems if there are hidden tabs in a page because it can't unselect items from the hidden tabs (still haven't figured out how do do that on another scraper I worked on). Finally, it's worth noting that this method of using CSS selectors is only going to work if the pages are structured in an orderly way. If not, you'll probably need to do more complex text mining.
